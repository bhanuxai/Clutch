...
import { transformItineraryToCalendarEvent } from "./src/lib/calendarTransformer";
import { formatTaskToGoogleEvent } from "./src/services/calendarService";
...
app.post("/api/tasks", (req, res) => {
  const { title, deadline, category, priority, estimatedMinutes, notes, tags } = req.body;
  if (!title || !deadline) {
...
app.post("/api/ai/prioritize", async (req, res) => {
  ...
  try {
    const prompt = `
      You are the core prioritization engine of the "Clutch" application.
...
app.post("/api/ai/schedule", async (req, res) => {
  ...
  try {
    const prompt = `
      You are the master scheduler for "Clutch".
...
app.post("/api/calendar/sync-task", async (req, res) => {
  ...
  try {
    const event = formatTaskToGoogleEvent(task);
    ...
    // Save Google Event ID on the task to mark it as synced
    task.googleEventId = responseData.id;
    saveData(data);
...
app.post("/api/calendar/sync-itinerary", async (req, res) => {
  ...
  try {
    ...
    for (const block of itinerary) {
      const event = transformItineraryToCalendarEvent(block);
      if (!event) continue;
...
app.post("/api/ai/quick-chat", async (req, res) => {
  ...
  try {
    const prompt = `
      You are the "Clutch" core agent. You help stressed students and professionals.
